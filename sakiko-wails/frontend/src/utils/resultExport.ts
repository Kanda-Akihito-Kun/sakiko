...
const exportPalettes: Record<ResolvedThemeMode, ExportPalette> = {
  light: {
    background: "#ffffff",
    watermark: "rgba(239, 112, 147, 0.045)",
    border: "#d8d8d8",
    divider: "#e5e5e5",
    headerFill: "#eeeeee",
    text: "#1f1f1f",
    mutedText: "#666666",
    latencyBase: "#d8f2f5",
    latencyAccent: "#45b7c5",
    speedBase: "#fde3eb",
    speedAccent: "#ef7093",
    errorText: "#8c1d40",
    errorFill: "#f5a3ba",
    mediaSuccessFill: "#c9ee8f",
    mediaWarningFill: "#ffd966",
    mediaErrorFill: "#f5a3ba",
    mediaNeutralFill: "#a9a9a9",
    rowEvenFill: "#ffffff",
    rowOddFill: "#f8f8f8",
    flagBadgeFill: "#f3f3f3",
    flagBadgeBorder: "#d4d4d4",
  },
  dark: {
    background: "#181a1f",
    watermark: "rgba(239, 112, 147, 0.08)",
    border: "#3b3f46",
    divider: "#2d3036",
    headerFill: "#2a2d33",
    text: "#f5f5f5",
    mutedText: "#b7bbc2",
    latencyBase: "#18373d",
    latencyAccent: "#47c2d1",
    speedBase: "#462632",
    speedAccent: "#f2799d",
    errorText: "#ffd7e1",
    errorFill: "#6a2d3e",
    mediaSuccessFill: "#355126",
    mediaWarningFill: "#5a4a1f",
    mediaErrorFill: "#6a2d3e",
    mediaNeutralFill: "#5d626b",
    rowEvenFill: "#1d2026",
    rowOddFill: "#23272f",
    flagBadgeFill: "#2a2e35",
    flagBadgeBorder: "#4a4f58",
  },
};
...
export function buildExportSections(archive: ResultArchive, privacy: ExportPrivacyOptions = {}): ExportSection[] {
  ...
  for (const section of archive.report.sections || []) {
    const normalized = applyPrivacyToSection(normalizeReportSection(section, archive), archive, resolvedPrivacy);
...
function normalizeReportSection(section: ResultReportSection, archive: ResultArchive): ExportSection {
  switch (section.kind) {
    case "speed_table": {
    ...
    case "topology_table":
      return {
        ...
        rows: sortTopologyRows(normalizeTopologyRows(section.rows || [], archive)),
      };
    case "udp_nat_table":
      return {
        kind: section.kind,
...
function drawHeader(
  ...
  const knightName = archiveKnightNameForDisplay(archive);
  const heading = knightName
    ? `Sakiko - ${mainTitle} | ${profileName} | ${knightName}`
    : `Sakiko - ${mainTitle} | ${profileName}`;

  ctx.fillStyle = palette.text;
  ctx.textAlign = "center";
  ctx.textBaseline = "middle";
  ctx.font = `700 24px ${FONT_FAMILY}`;
  ctx.fillText(heading, width / 2, PAGE_PADDING_Y + 20);
...
function drawFooter(
  ...
  ctx.font = `500 12px ${FONT_FAMILY}`;
  ctx.fillText(
    `Protocol Library=${protocolLibrary}  Backend=${backend}  SpeedTest Target=${target}  Profile=${maskedProfileNameForExport(archive, privacy) || "Unknown"}${knightName ? `  Knight=${knightName}` : ""}  Preset=${preset}`,
    PAGE_PADDING_X,
    footerTop + 18,
  );
  ctx.fillText(
...
function resolveCellFill(
  ...
  if (section.kind === "media_unlock_table" && key !== "nodeName" && key !== "proxyType") {
    switch (mediaCellTone(String(value || ""))) {
      ...
      default:
        return palette.mediaNeutralFill;
    }
  }
  return rowIndex % 2 === 0 ? palette.rowEvenFill : palette.rowOddFill;
}
...
function normalizeTopologyRows(rows: Record<string, unknown>[], archive: ResultArchive): Record<string, unknown>[] {
  const geoLookup = buildTopologyGeoLookup(archive);
  return rows.map((row) => {
    const proxyType = formatProxyType(row.proxyType);
    const key = buildResultKey(row.nodeName, proxyType, row.address);
    const geo = key ? geoLookup.get(key) : undefined;
    return {
      ...row,
      proxyType,
      inboundInfo: normalizeGeoInfo(row.inboundInfo, {
        country: firstText(row.inboundCountry, geo?.inbound.country),
        city: firstText(row.inboundCity, geo?.inbound.city),
        organization: firstText(row.inboundOrganization, extractGeoInfoOrganization(row.inboundInfo), geo?.inbound.organization),
      }),
      outboundInfo: normalizeGeoInfo(row.outboundInfo, {
        country: firstText(row.outboundCountry, geo?.outbound.country),
        city: firstText(row.outboundCity, geo?.outbound.city),
        organization: firstText(row.outboundOrganization, extractGeoInfoOrganization(row.outboundInfo), geo?.outbound.organization),
      }),
    };
  });
}

function normalizeGeoInfo(info: unknown, geo: { country?: string; city?: string; organization?: string }): string {
  const joined = joinGeoInfo(geo.country, geo.city, geo.organization);
  if (joined) {
    return joined;
  }
...
function joinGeoInfo(country: unknown, city: unknown, organization: unknown): string {
...
function buildTopologyGeoLookup(archive: ResultArchive): Map<string, { inbound: ExportGeoInfo; outbound: ExportGeoInfo }> {
  const lookup = new Map<string, { inbound: ExportGeoInfo; outbound: ExportGeoInfo }>();
  for (const result of archive.results || []) {
    ...
    lookup.set(key, {
      inbound: extractGeoInfo(result.matrices, "GEOIP_INBOUND"),
      outbound: extractGeoInfo(result.matrices, "GEOIP_OUTBOUND"),
    });
  }
  return lookup;
}
...
function extractGeoInfo(matrices: MatrixResult[] = [], targetType: string): ExportGeoInfo {
  const matrix = matrices.find((item) => item.type === targetType);
  if (!matrix || !matrix.payload || typeof matrix.payload !== "object") {
    return { country: "", city: "", organization: "" };
  }
  const payload = matrix.payload as Record<string, unknown>;
  return {
    country: firstText(payload.country, payload.countryCode),
    city: firstText(payload.city),
    organization: preferredGeoPayloadOrganization(payload),
  };
}

function preferredGeoPayloadOrganization(payload: Record<string, unknown>): string {
  const org = firstText(payload.asOrganization);
  const isp = firstText(payload.isp);
  if (!org) {
    return isp;
  }
  if (!isp) {
  ...
  if (isGenericGeoOrganization(org) && org.toLowerCase() !== isp.toLowerCase()) {
    return isp;
  }
  return org;
}

function isGenericGeoOrganization(value: string): boolean {
  const lower = value.trim().toLowerCase();
  return lower === "private customer"
    || lower === "private customers"
    || lower === "customer"
    || lower.includes("private customer");
}

function extractGeoInfoOrganization(value: unknown): string {
  const raw = String(value ?? "").trim();
  if (!raw) {
    return "";
  }
...
function archiveKnightNameForDisplay(archive: ResultArchive): string {
