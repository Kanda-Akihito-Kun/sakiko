...
export { extractProfileSubscriptionInfo, formatDataSize, formatDateTime, getFilteredNodes } from "./dashboardBasic";
...
export function formatMatrixPayload(payload: unknown, type?: string): string {
  ...
  if (payload && typeof payload === "object") {
    ...
    if (typeof record.type === "string" && ("publicIP" in record || "internalIP" in record)) {
      const type = String(record.type || "").trim();
      const internal = formatEndpoint(record.internalIP, record.internalPort);
      const external = formatEndpoint(record.publicIP, record.publicPort);
      const error = String(record.error || "").trim();
      const parts = [type, internal ? `Internal ${internal}` : "", external ? `Public ${external}` : ""].filter(Boolean);
...
export function formatMacroLabel(value?: string): string {
  switch ((value || "").trim().toUpperCase()) {
    ...
    case "UDP":
      return translate("shared.macros.udp", "UDP");
    case "SPEED":
...
export function formatMatrixLabel(value?: string): string {
  switch ((value || "").trim().toUpperCase()) {
    ...
    case "UDP_NAT_TYPE":
      return translate("shared.matrices.udpNatType", "UDP NAT Type");
    case "SPEED_AVERAGE":
...
export function formatTaskPresetLabel(value: TaskPreset): string {
  switch (value) {
    ...
    case "udp":
      return translate("shared.presets.udp", "UDP");
    case "speed":
...
function splitPresetGroups(preset?: string): string[] {
  ...
  if (normalized === "full") {
    return ["ping", "geo", "udp", "speed", "media"];
  }
...
export function summarizeResultMetrics(preset?: string): string {
  ...
  if (groups.includes("udp")) {
    labels.push(translate("shared.metrics.udp", "UDP NAT Type / Endpoint Mapping"));
  }
...
function formatEndpoint(ip: unknown, port: unknown): string {
  const host = String(ip || "").trim();
  const portValue = typeof port === "number"
    ? port
    : typeof port === "string"
