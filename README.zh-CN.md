...
核心执行模型保持为从 `miaospeed` 学习而来的模式：

`Vendor -> Macro -> Matrix`

这条边界很重要。业务逻辑放在 Go 中，而不是放在前端页面里。桌面端只是内核的消费方，不是核心测速逻辑的实现位置。
...
`sakiko-wails` 也提供了 `server` build tag，可用于简单的单机 demo 模式，但这并不是当前主产品形态。
