...
func (p *pollItem) Yield(idx int, c *taskpoll.Controller) {
	...
	for attempt := 1; attempt <= attempts; attempt++ {
		...
		candidate := executeNodeAttemptFunc(p.ctx, p.task, idx, p.matrices, p.macros, func(activeNode interfaces.TaskActiveNode) {
...
func executeNodeAttempt(ctx context.Context, task interfaces.Task, idx int, matrices []interfaces.MatrixEntry, macros []interfaces.MacroType, onUpdate func(interfaces.TaskActiveNode)) interfaces.EntryResult {
	...
	macroMap, err := runMacros(ctx, vendor, &task, matrices, macros, onUpdate)
	result.InvokeDuration = time.Now().UnixMilli() - start
	if err != nil {
		result.Error = err.Error()
...
func isMatrixFailed(target interfaces.MatrixType, matrices []interfaces.MatrixResult) bool {
	...
	case interfaces.MatrixUDPNATType:
		return false
