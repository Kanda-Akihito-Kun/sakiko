...
func (s *Service) Submit(task interfaces.Task, onEvent func(interfaces.Event)) (string, error) {
	...
	taskID, err := s.engine.Submit(task, executor.Callbacks{
		...
		OnProcess: func(taskID string, index int, result interfaces.EntryResult, queuing int) {
			state := s.completeTaskNode(taskID, index, total, queuing)
...
func (s *Service) GetTaskArchiveSnapshot(taskID string) (interfaces.TaskArchiveSnapshot, bool) {
	s.lock.RLock()
	defer s.lock.RUnlock()

	task, ok := s.tasks[taskID]
	if !ok {
		return interfaces.TaskArchiveSnapshot{}, false
	}

	return interfaces.TaskArchiveSnapshot{
		Task:     cloneTaskRecordTask(task.task),
		State:    task.state,
		Results:  append([]interfaces.EntryResult{}, task.results...),
		ExitCode: task.exitCode,
	}, true
...
func (s *Service) completeTaskNode(taskID string, nodeIndex int, total int, queuing int) interfaces.TaskState {
	s.lock.Lock()
	defer s.lock.Unlock()
	...
	if task.completed == nil {
		task.completed = map[int]struct{}{}
	}
	task.completed[nodeIndex] = struct{}{}
	task.state.Total = total
	task.state.Progress = len(task.completed)
...
func cloneTaskRecordTask(task interfaces.Task) interfaces.Task {
	cloned := task
