...
func TestTaskActivityTracksActiveNodes(t *testing.T) {
	...
	state = svc.completeTaskNode(task.ID, 0, 2, 3)
...
func TestTaskProgressCountsCompletedNodesOutOfOrder(t *testing.T) {
	t.Parallel()
	...
	state := svc.completeTaskNode(task.ID, 2, 3, 0)
	if state.Progress != 1 {
		t.Fatalf("expected progress 1 after first completed node, got %d", state.Progress)
	}

	state = svc.completeTaskNode(task.ID, 0, 3, 0)
	if state.Progress != 2 {
		t.Fatalf("expected progress 2 after second completed node, got %d", state.Progress)
	}

	state = svc.completeTaskNode(task.ID, 2, 3, 0)
	if state.Progress != 2 {
		t.Fatalf("expected duplicate completion to keep progress 2, got %d", state.Progress)
	}
}
