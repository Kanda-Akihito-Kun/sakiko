...
func NewResultStore(indexPath string) *ResultStore {
	return NewResultStoreForSubdir(indexPath, "results")
}

func NewResultStoreForSubdir(indexPath string, subdir string) *ResultStore {
	baseDir := filepath.Dir(indexPath)
	if baseDir == "." || baseDir == "" {
		baseDir = ""
	}

	resultDir := strings.TrimSpace(subdir)
	if resultDir == "" {
		resultDir = "results"
	}
	if baseDir != "" {
		resultDir = filepath.Join(baseDir, resultDir)
	}
...
func (s *ResultStore) SaveTaskArchive(snapshot interfaces.TaskArchiveSnapshot) error {
	...
	return s.SaveResultArchive(archive)
}

func (s *ResultStore) SaveResultArchive(archive interfaces.ResultArchive) error {
	if s == nil {
		return fmt.Errorf("result store is nil")
	}
	if strings.TrimSpace(archive.Task.ID) == "" {
		return fmt.Errorf("task ID is required")
	}
	if s.dir == "" {
		return fmt.Errorf("result directory is required")
	...
	if err := writeFileAtomic(s.Path(archive.Task.ID), raw, 0o644); err != nil {
		return err
	}
...
func buildResultArchiveListItem(archive interfaces.ResultArchive) interfaces.ResultArchiveListItem {
	knightName := ""
	if archive.Task.Environment != nil && archive.Task.Environment.Remote != nil {
		knightName = archive.Task.Environment.Remote.KnightName
	}
...
func buildResultReport(snapshot interfaces.TaskArchiveSnapshot) interfaces.ResultReport {
	...
	hasSpeedSection := hasAnyMatrix(snapshot.Task.Matrices, interfaces.MatrixAverageSpeed, interfaces.MatrixMaxSpeed, interfaces.MatrixPerSecSpeed, interfaces.MatrixTrafficUsed)
	if hasSpeedSection {
	...
	if hasAnyMatrix(snapshot.Task.Matrices, interfaces.MatrixUDPNATType) && !hasSpeedSection {
		sections = append(sections, buildUDPNATSection(snapshot))
	}
...
func buildSpeedSection(snapshot interfaces.TaskArchiveSnapshot) interfaces.ResultReportSection {
	includeUDPNATType := hasAnyMatrix(snapshot.Task.Matrices, interfaces.MatrixUDPNATType)
	...
	for _, result := range snapshot.Results {
		...
		if includeUDPNATType {
			nat, _ := extractUDPNATMatrix(result.Matrices, interfaces.MatrixUDPNATType)
			row["udpNatType"] = udpNATTypeCell(nat)
		}
	...
	if includeUDPNATType {
		columns = append(columns, interfaces.ResultReportColumn{Key: "udpNatType", Label: "UDP NAT Type"})
	}
	columns = append(columns,
		interfaces.ResultReportColumn{Key: "rttMillis", Label: "TLS RTT"},
		interfaces.ResultReportColumn{Key: "httpPingMillis", Label: "HTTPS Ping"},
		interfaces.ResultReportColumn{Key: "averageBytesPerSecond", Label: "Average Speed"},
		interfaces.ResultReportColumn{Key: "maxBytesPerSecond", Label: "Max Speed"},
		interfaces.ResultReportColumn{Key: "perSecondBytesPerSecond", Label: "Per-second Speed"},
		interfaces.ResultReportColumn{Key: "trafficUsedBytes", Label: "Traffic Used"},
		interfaces.ResultReportColumn{Key: "error", Label: "Error"},
	)
...
func buildTopologySection(snapshot interfaces.TaskArchiveSnapshot) interfaces.ResultReportSection {
	...
	for _, result := range snapshot.Results {
		...
		rows = append(rows, map[string]any{
			...
			"inboundCountry":       geoCountryCell(inbound),
			"inboundCity":          strings.TrimSpace(inbound.City),
			"inboundOrganization":  strings.TrimSpace(preferredGeoOrganization(inbound)),
			"inboundInfo":          geoInfoCell(inbound),
			"outboundIP":           outbound.IP,
			"outboundASN":          outbound.ASN,
			"outboundCountry":      geoCountryCell(outbound),
			"outboundCity":         strings.TrimSpace(outbound.City),
			"outboundOrganization": strings.TrimSpace(preferredGeoOrganization(outbound)),
			"outboundInfo":         geoInfoCell(outbound),
			"error":                rowError,
		})
	}
	...
	return interfaces.ResultReportSection{
		...
		Columns: []interfaces.ResultReportColumn{
			...
			{Key: "inboundIP", Label: "Inbound IP"},
			{Key: "inboundInfo", Label: "Inbound Info"},
			{Key: "outboundASN", Label: "Outbound ASN"},
			{Key: "outboundIP", Label: "Outbound IP"},
			{Key: "outboundInfo", Label: "Outbound Info"},
...
func buildUDPNATSection(snapshot interfaces.TaskArchiveSnapshot) interfaces.ResultReportSection {
	rows := make([]map[string]any, 0, len(snapshot.Results))
	successCount := 0

	for _, result := range snapshot.Results {
		nat, _ := extractUDPNATMatrix(result.Matrices, interfaces.MatrixUDPNATType)
		rowError := joinErrors(result.Error, nat.Error)
		if rowError == "" && strings.TrimSpace(string(nat.Type)) != "" {
			successCount++
		}
		...
		rows = append(rows, map[string]any{
			...
			"proxyType":        result.ProxyInfo.Type,
			"natType":          nat.Type,
			"internalEndpoint": formatEndpoint(nat.InternalIP, nat.InternalPort),
			"publicEndpoint":   formatEndpoint(nat.PublicIP, nat.PublicPort),
			"error":            rowError,
		})
	}
	...
	return interfaces.ResultReportSection{
		...
		Columns: []interfaces.ResultReportColumn{
			{Key: "nodeName", Label: "Node"},
			{Key: "proxyType", Label: "Protocol"},
			{Key: "natType", Label: "UDP NAT Type"},
			{Key: "internalEndpoint", Label: "Internal Endpoint"},
			{Key: "publicEndpoint", Label: "Public Endpoint"},
			{Key: "error", Label: "Error"},
		},
		Rows: rows,
		Summary: map[string]any{
...
func udpNATTypeCell(info interfaces.UDPNATInfo) string {
	if raw := strings.TrimSpace(string(info.Type)); raw != "" {
		return raw
	}
...
func geoInfoCell(info interfaces.GeoIPInfo) string {
	parts := make([]string, 0, 2)
	if location := geoLocationCell(info); location != "" {
		parts = append(parts, location)
	}
	if org := strings.TrimSpace(preferredGeoOrganization(info)); org != "" {
		parts = append(parts, org)
	}
	return strings.Join(parts, " | ")
}

func geoLocationCell(info interfaces.GeoIPInfo) string {
	locationParts := make([]string, 0, 2)
	if country := geoCountryCell(info); country != "" {
		locationParts = append(locationParts, country)
	}
	if city := strings.TrimSpace(info.City); city != "" && !strings.EqualFold(city, geoCountryCell(info)) {
		locationParts = append(locationParts, city)
	}
	return strings.Join(locationParts, "-")
}

func geoCountryCell(info interfaces.GeoIPInfo) string {
...
func extractUDPNATMatrix(matrices []interfaces.MatrixResult, target interfaces.MatrixType) (interfaces.UDPNATInfo, bool) {
	for _, matrix := range matrices {
		if matrix.Type != target {
			continue
		}

		payload, ok := decodePayload[interfaces.UDPNATInfo](matrix.Payload)
		if ok {
			return payload, true
		}
	}
	return interfaces.UDPNATInfo{}, false
}
