...
func TestBuildMediaUnlockSectionFiltersRemovedPlatforms(t *testing.T) {
				...
				Matrices: []interfaces.MatrixResult{
					{
						...
						Payload: interfaces.MediaUnlockResult{
							Items: []interfaces.MediaUnlockPlatformResult{
								...
								{
									Platform: interfaces.MediaUnlockPlatformBilibiliTW,
									Name:     "Bilibili Taiwan",
									Status:   interfaces.MediaUnlockStatusYes,
									Display:  "Unlocked (TW)",
								},
								{
	...
	for _, column := range section.Columns {
		if column.Key == "dazn" || column.Key == "instagram_music" || column.Key == "hulu_jp" || column.Key == "spotify" || column.Key == "steam" || column.Key == "bilibili_tw" {
	...
	if _, exists := section.Rows[0]["bilibili_tw"]; exists {
		t.Fatalf("expected Bilibili TW cell to be omitted from report row")
	}
...
func TestBuildTopologySectionUsesCompactASNAndInfoColumns(t *testing.T) {
		...
		Results: []interfaces.EntryResult{
			{
				ProxyInfo: interfaces.ProxyInfo{Name: "node-a", Type: interfaces.ProxyShadowsocks},
				Matrices: []interfaces.MatrixResult{
					{Type: interfaces.MatrixInboundGeoIP, Payload: interfaces.GeoIPInfo{IP: "59.61.129.169", ASN: 9808, ASOrganization: "China Mobile", Country: "China", City: "Wuhan"}},
					{Type: interfaces.MatrixOutboundGeoIP, Payload: interfaces.GeoIPInfo{IP: "203.0.113.8", ASN: 2516, ASOrganization: "KDDI", Country: "Japan", City: "Tokyo"}},
				},
			},
		},
	})
	...
	if got := section.Rows[0]["inboundInfo"]; got != "China-Wuhan | China Mobile" {
	...
	if got := section.Rows[0]["outboundInfo"]; got != "Japan-Tokyo | KDDI" {
