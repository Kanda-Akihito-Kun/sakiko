...
func (m *Macro) Run(ctx context.Context, proxy interfaces.Vendor, task *interfaces.Task) error {
	...
	rtt, delay := ping(ctx, proxy, cfg.PingAddress, cfg.PingAverageOver, int(cfg.TaskRetry), cfg.TaskTimeoutMillis)
	m.RTT = rtt
	m.Delay = delay
...
func ping(ctx context.Context, proxy interfaces.Vendor, rawURL string, avg uint16, retry int, timeoutMS uint) (uint16, uint16) {
	if proxy == nil {
		return 0, 0
	}
	if ctx == nil {
	...
	for sampleIndex := 0; sampleIndex < int(avg); sampleIndex++ {
		...
		rtt, delay, ok := retryPingSample(ctx, proxy, rawURL, timeoutMS, retry, sampleIndex+1, int(avg))
		if !ok {
			continue
		}
...
func retryPingSample(
	...
	for attempt := 1; attempt <= attempts; attempt++ {
		...
		rtt, delay, err := measurePingSample(attemptCtx, proxy, rawURL)
		cancel()
